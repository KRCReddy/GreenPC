...
app.get('/api/products', (req, res) => {
  ...
  if (q.ram) products = products.filter(p => (p.ram_gb || 0) >= Number(q.ram));
  if (q.storageType) products = products.filter(p => (p.storage_type || '').toLowerCase() === q.storageType.toLowerCase());
...
app.post('/api/inquiries', (req, res) => {
  ...
  const entry = { id: uuidv4(), ...req.body, created_at: new Date().toISOString() };
...
function isAdmin(pass) { return pass === ADMIN_PASS; }
...
app.post('/api/admin/products', (req, res) => {
  ...
  if (!incoming.id) incoming.id = 'p-' + uuidv4();
